...
local function setLabel()
	local label = PlayerUI.GetAutoMergeLabel()
	if label and label:IsA("TextLabel") then
		label.Text = enabled and "Auto Merge On" or "Auto Merge"
	end
end

local function updateVisual()
	local button = PlayerUI.GetAutoMergeButton()
	if button then
...
local function syncServer()
	pcall(function()
		toggleFn:InvokeServer({ merge = enabled })
	end)
end
...
local function bind()
	...
	if button:GetAttribute("auto_merge_bound") then
		return
	end
	button:SetAttribute("auto_merge_bound", true)
	enabled = false
	updateVisual()
